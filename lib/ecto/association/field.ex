defmodule Ecto.Schema.Field do
  ...
  def define_field(mod, name, type, opts) do
    ...
    if not virtual? do
      ...
      if opts[:writable] != :always && gen do
  ...
  def define_field3(mod, name, type, opts) do
    put_struct_field(mod, name, Keyword.get(opts, :default))
    ...
    if not opts[:virtual] do

      #opt_in(:autogenerate, opts[:autogenerate], mod, name, type, opts)
      case gen = opts[:autogenerate] do
        {_, _, _} ->
          store_mfa_autogenerate!(mod, name, type, gen)
      ...
      opt_in(:read_after_writes, mod, name, type, opts)
      # if opts[:read_after_writes] && opts[:autogenerate] do
      #   raise ArgumentError, "cannot mark the same field as autogenerate and read_after_writes"
      # end

      opt_in(:writable, mod, name, type, opts)
      opt_in(:primary_key, mod, name, type, opts)
      opt_in(:load_in_query, mod, name, type, opts)
  ...
  def opt_in(:writable, writable, mod, _name, _type, opts) do
  ...
  def opt_in(:source, source, mod, name, _type, opts) when is_atom(source) do
    if not opts[:virtual] and name != source do
      Module.put_attribute(mod, :ecto_field_sources, {name, source})
    end
    mod
  end

  def opt_in(:source, source, mod, name, _type, opts) when not is_atom(source) do
    if not opts[:virtual] do
      raise ArgumentError,
  ...
  def opt_in(:load_in_query, true, mod, name, type, opts) do
    if not(opts[:virtual] || false) do
      Module.put_attribute(mod, :ecto_query_fields, {name, type})
    end
    mod
  end

  def opt_in(:load_in_query, false, mod, _, _, _opts) do
    mod
  end
  ...
  def opt_in(_option, _value, mod, _name, _type, _opts) do
    mod
  end
